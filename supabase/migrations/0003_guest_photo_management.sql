...
--
-- Granted to both anon and authenticated: a guest's browser can carry an
-- authenticated Supabase session (e.g. the host previewing their own event's
-- guest link while signed in), which makes the request run as `authenticated`
-- rather than `anon` — same guest-id-in-cookie trust model either way.
-- ============================================================
